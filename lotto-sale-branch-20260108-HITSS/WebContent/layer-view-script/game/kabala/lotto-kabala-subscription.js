...
setup: function () {
  //combo_fecha = $(".selectBox").html();
    ...
    $('#J1-check-chauchamba').prop("disabled", true);
...
var $kabalaSubscription = function () {
  run.setup();
    ...
    $(".clear").click(function () {
        if (status == "ACT") {
            ...
            combinatoria();
            if(dataLayerlimpiar){
            	datalayerJugada(this,'Elige tu Combo Kábala');
            }
        }
    });
    $(".azar").click(function () {
      ...
      $(".left-panel").on("click",".process-delete",function(){
    	try {
    	   tagginKabalaRemoveFromCart(content_object[0]);				
    	   datalayerRemoveFromCart(content_object[0],content_object[0][6]/content_object[0][9]);
		} catch (e) {
			console.error(e);
		}
    	  
    	  window.location.href = "juega-kabala.html";
    	/********  
      ...
      $("#content-subscribe-grid-result").html("");
        ...
        $("#btn_desktop_combo_kabala_azar_simple").removeClass("btn-active");        
        $("#btn_desktop_combo_kabala_misma_jugada").removeClass("btn-active");
      ...
      $('.left-panel').on('click','#btn-no-process',function(){
        ...
        /*for (i in content_object) {
          ...
          var temp_jugada_2 = "<span class='label_1'>SUSCRIPCI&Oacute;N KABALA POR ADELANTADO</span><div id='content-subscribe-grid-result'></div><div id='num_subscribe_link'></div>";
    ...
    $(document).on("click", "#btn_desktop_combos_kabala", function(){
      clear_all();
      $(".panel-subscription").addClass("disabled");
      $(".panel-subscription-book").removeClass("disabled");
      try {
    	  tagginEligeJugadaComboKabala();			
    	  datalayerBtnsTipoJuego(this,'¡Combos Kábala! No te pierdas ni un sorteo');
		} catch (e) {
			console.error(e);
		}
    ...
    $(document).on("click", "#btn_desktop_juega_kabala", function(){
        datalayerBtnsTipoJuego(this,'¡Combos Kábala! No te pierdas ni un sorteo');
        window.location.href = 'juega-kabala.html?state=1';
      });
    //$(document).on("click", "#same-play", function(e){
    $(document).on("click", "#btn_desktop_combo_kabala_misma_jugada", function(e){	
      e.preventDefault();
      clear_all();
      ...
      //$("#random-play").removeClass("btn-active");
      $("#btn_desktop_combo_kabala_misma_jugada").addClass("btn-active");
      $("#btn_desktop_combo_kabala_azar_simple").removeClass("btn-active");
      datalayerJugada(this,'Elige tu Combo Kábala');
    });
    //$(document).on("click", "#random-play", function(e){
    $(document).on("click", "#btn_desktop_combo_kabala_azar_simple", function(e){
      e.preventDefault();
      clear_all();
      $('.subscription-disabled-sec1').addClass('subscription-disabled');
      $('.subscription-disabled-sec2').removeClass('subscription-disabled');
      ...
      //$("#same-play").removeClass("btn-active");
      $("#btn_desktop_combo_kabala_azar_simple").addClass("btn-active");
      $("#btn_desktop_combo_kabala_misma_jugada").removeClass("btn-active");
      datalayerJugada("Jugada simple al 'Azar' para cada sorteo *",'Elige tu Combo Kábala');
    });
    $(document).on("click", ".btn-discount10", function(){
      ...
      $("#inp_desktop_combo_12_sorteos_kabala").prop("checked",true);
      combinatoria();
    });
    $(document).on("click", ".btn-discount20", function(){
      ...
      $("#inp_desktop_combo_36_sorteos_kabala").prop("checked",true);
      combinatoria();
    });
    $(document).on("click", ".btn-discount30", function(){
      ...
      //$("#discount30").prop("checked",true);
            $("#inp_desktop_combo_72_sorteos_kabala").prop("checked",true);
      combinatoria();
    });
    $(document).on("click", ".btn-discount40", function(){
      ...
      $("#inp_desktop_combo_144_sorteos_kabala").prop("checked",true);
      combinatoria();
    });
  ...
  $(document).on("click","#btn_desktop_comprar_combo_kabala", function () {
		...
		mainValidatePendingsDocsForAproval('lottoKabalaSubscriptionComprarComboKabala');
  });
  //$(document).on("click", "#btn_finaliza_compra", function () {
  var thisBtnFinalizarCompraComboKabala = null;
  $(document).on("click", "#btn_desktop_finalizar_compra_combo_kabala", function () {
	  	thisBtnFinalizarCompraComboKabala = this;
	  	// Se valida si el usiuario tiene documentos pendientes de confirmación, el parámetro que recibe
		// es una function que se realizará en caso no tenga docs pendientes de confirmacion
		mainValidatePendingsDocsForAproval('lottoKabalaSubscriptionFinalizarCompraComboKabala');
  });
  
  function finalizarCompraComboKabala() {
      ...
      if(message == "OK") {
          ...
          if(content_object.length != 0) {
              ...
              $("#ajax-loader").show();
              var contador = 0;
              datalayerFinalizaCompra2(thisBtnFinalizarCompraComboKabala,'Finalizar Compra');
              datalayerCheckout(content_object[0],content_object[0][6]/content_object[0][9]);
              $.ajax({type: "POST", url: "jugadasSuscripcionKabala.html", data: "dato=" + result_ticket, dataType: "text", success: function (e) {
                  ...
                  //for (var x in new_content_object) {
                      ...
                      if (dato_proceso[1] == "null") {
                          ...
                          var game_no_process_info = "<div class='title-text text-autocontrol'>" + "<div>Tienes jugadas que no se han podido procesar.</div></div>" + "<div id='no-process-section'><a href='#' class='button button-block button-dark-green' id='btn-no-process' onclick='return false;'><b>VOLVER</b></a></div>";
                          ...
                          try {
                          	if( dato_proceso[0].includes("CLIENTE NO EXISTE") == true ){
                          		taggingKabalaJugadaNoProcesada("No se ha encontrado el registro del cliente");
                          	}else if( dato_proceso[0].includes("CREDITO INSUFICIENTE") == true || dato_proceso[0].includes("Cuenta Lotocard ha expirado") == true ){
                          		taggingKabalaJugadaNoProcesada("No cuenta con saldo disponible para realizar este proceso");
                          	}else{
                          		taggingKabalaJugadaNoProcesada("Ocurrio un error intente nuevamente");
                          	}
							} catch (e) {
								console.error(e);
							}
                          ...
                          try {
                          	kabalaTagginEEpurchase(new_content_object);	
              			} catch (e) {
              				console.error(e);
              			}
  ...
  function comprarComboKabala(){
	//alert("value_number1="+value_number1+" length="+value_number1.length+" total="+total+" costoTotalBet="+costoTotalBet);
	    if(status=="ACT"){
	      ...
	      if(!$("#btn_desktop_combo_kabala_misma_jugada").hasClass("btn-active") && !$("#btn_desktop_combo_kabala_azar_simple").hasClass("btn-active")) {
	      ...
	      //} else if($("#same-play").hasClass("btn-active") && (total<1 || percent==0)) {
	      } else if($("#btn_desktop_combo_kabala_misma_jugada").hasClass("btn-active") && (total<1 || percent==0)) {
	    	  ...
	    	  try {
	    		  ...
	    		  tagginPopupError("Kabala",errorTagging);	
	    		  datalayerErrores('Finalizar tu Compra 1','Paso 2','Comprar',label);
				} catch (e) {
					console.error(e);
				}
	      ...
	      //} else if($("#random-play").hasClass("btn-active") && percent==0) {
	      } else if($("#btn_desktop_combo_kabala_azar_simple").hasClass("btn-active") && percent==0) {
	        ...
	        jAlert(label, null);
	      /*if((0<value_number1.length && value_number1.length<6)){
	        jAlert("Elige correctamente tu apuesta.", null);                     
	      } else if (total<1 && costoTotalBet<=0) {*/
	      } else {
	    	  try {
	    		  tagginEEaddToCart("Kabala",consecutive,costoTotalBet);		
				} catch (e) {
					console.error(e);
				}
...
//	    	  setCaptchaJuego();
	        ...
	        object_boleto.push(value_number1);
	          ...
	          if($("#btn_desktop_combo_kabala_misma_jugada").hasClass("btn-active")){
	        	  object_boleto.push(1);
	          }
	          if($("#btn_desktop_combo_kabala_azar_simple").hasClass("btn-active")){
	          ...
	          try {
	              ...
	              datalayerFinalizaCompra1($('#btn_desktop_comprar_combo_kabala'),'Comprar');
	          } catch (e) {
				console.error(e);
	          }
  ...
  window.addEventListener("message", function(event) {
      if (event.data === "lottoKabalaSubscriptionComprarComboKabala") {
          comprarComboKabala(); 
      } else if (event.data === "lottoKabalaSubscriptionFinalizarCompraComboKabala") {
    	  finalizarCompraComboKabala(); 
      }
  });
...
function combinatoria() {
  var sub_cantin1 = 0;
    ...
    /*if(sub_cantin1==0) {
      ...
      if(sub_total <= 5005) total = cantin_1 + cantin_2 + cantin_3 + cantin_4;
    }*/
    ...
    if($("input[name=discount]").length>0) {
      ...
      if(price_base != "") {
          ...
          dis10 = $("#inp_desktop_combo_12_sorteos_kabala").data("discount").split("_");
          dis20 = $("#inp_desktop_combo_36_sorteos_kabala").data("discount").split("_");
          dis30 = $("#inp_desktop_combo_72_sorteos_kabala").data("discount").split("_");
          dis40 = $("#inp_desktop_combo_144_sorteos_kabala").data("discount").split("_");          
          ...
          $("#inp_desktop_combo_12_sorteos_kabala").text(peformat(pricedis10));
          $("#inp_desktop_combo_36_sorteos_kabala").text(peformat(pricedis20));
          $("#inp_desktop_combo_72_sorteos_kabala").text(peformat(pricedis30));
    ...
    if(($("#btn_desktop_combo_kabala_misma_jugada").hasClass("btn-active") && total >= 1 && subscription.length>0)||($("#btn_desktop_combo_kabala_azar_simple").hasClass("btn-active") && subscription.length>0)) {
        //if(total==0 && subscription.length>0 && $("#random-play").hasClass("btn-active")){
        if(total==0 && subscription.length>0 && $("#btn_desktop_combo_kabala_azar_simple").hasClass("btn-active")){
        	total = 1;
        }
    	//if(total >= 1) {
      ...
      consecutive = (subscription=="inp_desktop_combo_12_sorteos_kabala")?draws10:(subscription=="inp_desktop_combo_36_sorteos_kabala")?draws20:(subscription=="inp_desktop_combo_72_sorteos_kabala")?draws30:(subscription=="inp_desktop_combo_144_sorteos_kabala")?draws40:1;
      sorteos = (subscription=="inp_desktop_combo_12_sorteos_kabala")?total*draws10:(subscription=="inp_desktop_combo_36_sorteos_kabala")?total*draws20:(subscription=="inp_desktop_combo_72_sorteos_kabala")?total*draws30:(subscription=="inp_desktop_combo_144_sorteos_kabala")?total*draws40:1;
      ...
      costoTotalBetMensaje = (subscription=="inp_desktop_combo_12_sorteos_kabala")?priceundis10:(subscription=="inp_desktop_combo_36_sorteos_kabala")?priceundis20:(subscription=="inp_desktop_combo_72_sorteos_kabala")?priceundis30:(subscription=="inp_desktop_combo_144_sorteos_kabala")?priceundis40:0;
      ...
      costoTotalBet = (subscription=="inp_desktop_combo_12_sorteos_kabala")?pricedis10:(subscription=="inp_desktop_combo_36_sorteos_kabala")?pricedis20:(subscription=="inp_desktop_combo_72_sorteos_kabala")?pricedis30:(subscription=="inp_desktop_combo_144_sorteos_kabala")?pricedis40:0;      
      $("#price-info").text((subscription=="inp_desktop_combo_12_sorteos_kabala")?months10:(subscription=="inp_desktop_combo_36_sorteos_kabala")?months20:(subscription=="inp_desktop_combo_72_sorteos_kabala")?months30:(subscription=="inp_desktop_combo_144_sorteos_kabala")?months40:0);
      ...
      $("#price-discount").text((subscription=="inp_desktop_combo_12_sorteos_kabala")?percent10:(subscription=="inp_desktop_combo_36_sorteos_kabala")?percent20:(subscription=="inp_desktop_combo_72_sorteos_kabala")?percent30:(subscription=="inp_desktop_combo_144_sorteos_kabala")?percent40:0);
...
function process_add_ons() {
    ...
    var plus = " KB P+";//' KB Plus+';
    var chauChamba = " KB CC"//' KB Chau Chamba';
    $('#plus-text').html('');
    ...
    if ($('#J1-check-chauchamba').prop('checked') && cantin_1 > 0) {
        total_chauchambaJ1 = cantin_1;// * 1;
        add_oncJ1 = 'AD2';
        //$('#chau-chamba-text').html(' / ' + chauChamba);
    }
...
function tagginEligeJugadaComboKabala(){
	window.dataLayer = window.dataLayer || [ ];
	dataLayer.push({
	    'event': 'virtualPageView',
	    'pageUrl': '/juega-kabala/combo/paso1/',
	    'pageTitle': 'Elige tu jugada - Combo - Kábala'

	});
	
	var tag="Kabala - Combo - Paso 1 - Elige tu jugada";
...
function tagginFinalizar(){
	...
	dataLayer.push({
	    ...
	    'pageTitle': 'Finaliza tu compra - Combo - Kábala'

	});
	
	var tag="Finaliza tu compra - Paso 2 -Combo - Kabala";
	console.log("Taggin event: "+tag);
	...
	if(nCombos!=undefined && nCombos!=null && nCombos!=''){
		...
		'event': 'EEcheckout',
		    'ecommerce': {
		      'checkout': {
		        'actionField': {'step': 1},
		        'products': [{ //Acá se listan todos los productos individuales
		          'name': 'Kabala Combo '+nCombos+' jugadas',
		          'id': 'kabalaC'+nCombos,
		          'price': price,
		          'brand': 'Juegos',
		          'quantity': '1',
		...
		var tag="EEcheckout Kabala";
		console.log("Taggin event: "+tag+", Combo: "+nCombos+", precio: "+price);
	}
}

function tagginKabalaRemoveFromCart(datos){
	var nCombos=datos[5];
	if(nCombos!=undefined && nCombos!=null && nCombos!=''){
		...
		var tag="Kabala combo EEremoveFromCart";
		console.log("Taggin event: "+tag+", Combo: "+nCombos+", precio: "+price);
	}
}
...
function tagginKabalaCompraExitosa(){
	...
	var tag="Kabala combo compraExitosa";
	console.log("Taggin event: "+tag);

}

function kabalaTagginEEpurchase(content_object){
	...
	if(nCombos!=undefined && nCombos!=null && nCombos!=''){
		...
		'event': 'EEpurchase',
		  'ecommerce': {
		    'purchase': {
		      'actionField': {
		        'id': 'KB-'+idTransaction,               //{transactionID} código de la transacción
		         'revenue': price,    //{revenue} Monto total pagado
		      },
		      'products': [{  
		          'name': 'Kabala Combo '+nCombos+' jugadas',
		          'id': 'kabalaC'+nCombos,
...
function taggingKabalaJugadaNoProcesada(mensaje){
	try{
		window.dataLayer = window.dataLayer || [ ];
		dataLayer.push({
		...
		console.log("taggingKabalaJugadaNoProcesada: mensaje:"+mensaje);
	}catch(e){
		console.error(e);
	}
}
...
var swfobject=function(){
	...
	var h=function(){
		...
		if(typeof T.plugins!=b&&typeof T.plugins[n]==Q){
			...
			if(typeof j.ActiveXObject!=b){
				var y=null,AB=false;
				try{
					y=new ActiveXObject(p+".9")
				}catch(t){
					try{
						y=new ActiveXObject(p+".10");
						AC=[9,0,28,0];
	...
	var L=function(){
		...
		if(h.webkit&&typeof K.readyState!=b){
			Z=setInterval(function(){if(/loaded|complete/.test(K.readyState)){E()}},10)
		}
			...
			if(h.pv[0]>0){
				...
				if(r){
					...
					if(c(N[q].swfVersion)){
						...
						if(N[q].expressInstall&&!A&&c("9,0,28,0")&&(h.win||h.mac)){
							k(N[q])
						}else{
							O(r)
						}
	...
	return{
		...
		embedSWF:function(x,AE,AB,AD,q,w,r,z,AC){
			...
			if(c(q)){
				...
				if(w&&!A&&c("9,0,28,0")&&(h.win||h.mac)){
					A=true;
					W(AE,false);
					f(function(){var AF={};AF.id=AF.altContentId=AE;AF.width=AB;AF.height=AD;AF.expressInstall=w;k(AF)})
				}
			}
		},

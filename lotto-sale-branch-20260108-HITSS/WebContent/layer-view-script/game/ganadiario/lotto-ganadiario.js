...
//                    });                
                ...
                for (var v in content_object_2) {
                	var precio_cu = parseFloat(content_object_2[v][6]);
                ...
                value_numberJ3 = [];
                value_numberJ4 = [];
                tagginGanaDiarioEEaddToCart(costoTotalBet);
                ...
                let precioUnitario = (content_object_2[pos][8]/content_object_2[pos][9])/content_object_2[pos][5];
                datalayerAddToCart(content_object[pos],precioUnitario);
                datalayerFinalizaCompra1($('#btn_desktop_comprar_boleto_ganadiario'),'Comprar');
                
            }
            if (total > 3003) {
            	datalayerErrores('Finalizar tu Compra 1','Paso 2','Comprar','El total de apuestas no debe ser mayor a 3003');	
                jAlert("El total de apuestas no debe ser mayor a 3003", null)
            }
        }
    }
...
//    	   setCaptchaJuego();
    ...
    $('#btn_desktop_comprar_boleto_ganadiario').on('click', function () {
        if (status == 'ACT') {
...
//            }
        	...
        	mainValidatePendingsDocsForAproval('lottoGanadiarioBuy');
        }
    });
    ...
    function changeSelectedBox(){
    	var sorteosLocal = $("#mySelectBox").val();
    	if (sorteosLocal == null) {
            sorteos = 1
        } else {
        	sorteos = $(this).val();
    		combinatoria()
    ...
    function callTransformType7x5(p_sum_total_bet, p_number_consecutive) {
        ...
        if (p_number_consecutive >= 7)var_total_cost = ((var_sum_total_bet_consecutive * v_data_value_bet) - (Math.floor(p_number_consecutive / 7) * 2));
        return var_total_cost
    }
    ...
    $('.left-panel').on('click','.process-delete1',function(){
        ...
        tagginGanaDiarioEEremoveFromCart(content_object_2[pos][6]);
        let precioUnitario = (content_object_2[pos][8]/content_object_2[pos][9])/content_object_2[pos][5];
    ...
    var thisBtnFinalizarCompraGanadiario = null;
    $("#btn_desktop_finalizar_compra_ganadiario").click(function () {
    	thisBtnFinalizarCompraGanadiario = this;
    	// Se valida si el usiuario tiene documentos pendientes de confirmación, el parámetro que recibe
    	// es una function que se realizará en caso no tenga docs pendientes de confirmacion
    	mainValidatePendingsDocsForAproval('lottoGanadiarioFinalizarCompraGanadiario');
    });
...
//                array_no_procesados.push(new_content_object[i]);
    ...
    function finalizarCompraGanadiario(){
        ...
        if (message == "OK") {
        	datalayerFinalizaCompra2(thisBtnFinalizarCompraGanadiario,'Finalizar Compra');
        	let precioUnitario = (content_object_2[0][8]/content_object_2[0][9])/content_object_2[0][5];
            ...
            if (content_object.length != 0) {
                $(".left-panel").html("");
                var content_grid_2 = "<div class='label-top'></div>" + "<div class='label_1'>GANA DIARIO</div>" + "<div id='ajax-loader'>" + "<img src='layer-view-image/game/ganadiario/ajax-loader.gif'>" + "</div><div id='content-grid-result'></div>" + "<div id='num_link'></div>";
                ...
                $("#ajax-loader").show();
                var contador = 0;
                $.ajax({type: "POST", url: "ajaxJugadas.html", dataType: "text", data: "dato=" + result_ticket, success: function (e) {
                    ...
                    $(".label_resu3").html("<b>Saldo Disponible: <div class='text-detail-pay detail-pay-monto'>S/ " + floatFormat(new_content_object[(new_content_object.length - 1)][10]) + "</div>");
                    var promotional_balance = parseFloat(new_content_object[(new_content_object.length - 1)][11]);
                    if (promotional_balance > 0) {
                        $(".label_resu4").html("<b>Saldo Promocional: <div class='text-detail-pay detail-pay-monto'>S/ " + floatFormat(new_content_object[(new_content_object.length - 1)][11]) + "</div>");
                    }
                    ...
                    if(!$('.label-top').find('div').length){
                    	...
                    	tagginGanaDiarioEEpurchase(content_object, new_content_object);
                    	...
                    	for(let pos=0;pos<content_object_2.length;pos++){
    						...
    						let precioUnitario = (parseFloat(content_object[pos][8])/parseFloat(content_object[pos][9]))/parseFloat(content_object[pos][5]);
    						
                        	datalayerPurchase(content_object[pos],
                    						precioUnitario,
                    					  '¡Gracias por tu Compra!',
                    					  costo_total3,
                    					  'GanaDiario-'+procesos[1],
                    					  cupon);
                    	}
                    }
                }})
    ...
    window.addEventListener("message", function(event) {
        if (event.data === "lottoGanadiarioBuy") {
            buy(); 
        } else if (event.data === "lottoGanadiarioFinalizarCompraGanadiario") {
        	finalizarCompraGanadiario(); 
        }
    });
    ...
    function grilla_boletos2(data) {
        ...
        if($(".label-top > div").text()==='JUGADAS NO PROCESADAS'){
        	tagginJugadaNoProcesada("Gana Diario", label);
        }
    }
...
function tagginGanaDiarioEEaddToCart(costoTotalBet){
	try {
		...
		var tag="Gana Diario Jugada Individual EEaddToCart";
...
function tagginFinalizar(){
	try {
		...
		var tag="Finaliza tu compra - Individual - Gana Diario";
...
function tagginGanaDiarioEEcheckout(){
	try {
		...
		var tag="EEcheckout - Individual - Gana Diario";
		console.log("Taggin event: "+tag);				
	} catch (e) {
		console.error(e);
	}
...
function tagginGanaDiarioEEremoveFromCart(price){
	try {
		price=price.toFixed(2);
		window.dataLayer = window.dataLayer || [ ];
		dataLayer.push({
		...
		var tag="EEremoveFromCart - Individual - Gana Diario";
...
function tagginGanaDiarioCompraExitosa(){
	try {
		...
		var tag="Gana Diario  compraExitosa";
		console.log("Taggin event: "+tag);				
	} catch (e) {
		console.error(e);
	}
...
function tagginGanaDiarioEEpurchase_1(content_object, new_content) {
	try {
		var idTransaction = 0;
		var total_price = 0;
		var aproduct = [];
		var idTransaction = "GD";
		...
		var tag = "Gana Diario  EEpurchase";
		console.log("Taggin event: " + tag);				
	} catch (e) {
		console.error(e);
	}
...
function tagginGanaDiarioEEpurchase(content_object, new_content) {
	try {
		var idTransaction = 0;
		var total_price = 0;
		var aproduct = [];

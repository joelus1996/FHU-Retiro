...
const formTransferencia = (function () {
	  const form = $('#formtransferencia'),
	    ...
	    send = function (e) {
	        e.preventDefault();
	        const valid = form.formValidate();
	        console.log("formTransferencia:" + valid);
	        tipoTransferencia="";
	     ...
	     changeInputNumCuenta = function() {
	    	 datalayerCobrarPremioMetodoCobroIngNumCuenta($("#inumacount"),'Transferencia Bancaria');
	     },
...
function ingresarMetadatos(cid){
	  var metadata = '{"plataform": "EC-DESKTOP","clientId":"' + cid + '","fixedLanguage":"es"}';
	  ...
	  */
	  ...
	  setTimeout(function() {
		  ...
		  if (get == 0){
			  ...
			  console.log("consulta automática nro 2");
			  setTimeout(function() {
				  get = getResultKycV2("SAVE");
				  if(get == 0){
					  //activar consulta manual
					  console.log("Activación manual");
...
function onChangeAmountEfectivo(){
	...
	datalayerCobrarPremioMetodoCobroIngresarMonto($("#amountEfectivo"),'Efectivo / Punto de venta');
...
function onChangeAmountVisa(){
	//comentar dni
	datalayerCobrarPremioMetodoCobroIngresarMonto($("#amountVisa"),'Visa');
...
function onChangeAmountTransferencia(){
	...
	datalayerCobrarPremioMetodoCobroIngresarMonto($("#amountTransferencia"),'Transferencia Bancaria');
	if(isDniActive){
					...
					if(data.listPrizesMajor.length>0){
						...
						$.each(JSON.parse(data.listPrizesMajor), function( key,value ) {
							...
							if(value.gameId==4){//GG
								...
								htmlListaPremiosMayoresLoterias+='<td style="min-width: 130px;"><span style="padding-top: 12px; font-family: Avenir; font-weight: 900; padding-left: 30px; font-size: 18px;">S/ '+floatFormat(value.prizeAmount).replace(".00","")+'</span></td>';
...
function downloadTicketsPPDebitIdQR(){
	document.getElementById("formDownloadDebitIdQR").submit();
}
		...
		if(framePago==null || framePago==undefined){//si ya tokenizo
			...
			.done(function(data) {
				...
				if(data.status=="OK"){
					...
					$.ajax({
                        type: "post",
                        url: "registerCAPP.html",
                        dataType: 'json',
                        async: false,
                        data: "&medio=NIUBIZ&code="+data.code+"&description="+data.description+"&monto"+$('#amountVisa').val(),
                        success: function (data) {
                        	if(data.ban!=undefined && data.ban=="OK"){
        						...
        						deleteTokenizedCard();
        						showMessageErrorVisa(title_error_general,messageError);
        					}
                        },
                        error: function () {
                        	console.error("error registerCAPP");
                        },
    				});
				}
				$('body').find('#loadTokenization').remove();
...
function verTicketPM(){
	if(premioSeleccionado.gameId==4){//GG
		openPreviewWindowPP(premioSeleccionado.gameId,"Ganagol",premioSeleccionado.ticketId);
	}else if(premioSeleccionado.gameId==41){//TK
		openPreviewWindowPP(premioSeleccionado.gameId,"Tinka",premioSeleccionado.ticketId);
	}else if(premioSeleccionado.gameId==42){//KB
		openPreviewWindowPP(premioSeleccionado.gameId,"Kábala",premioSeleccionado.ticketId);
	}else if(premioSeleccionado.gameId==164){//GD
		openPreviewWindowPP(premioSeleccionado.gameId,"Gana Diario",premioSeleccionado.ticketId);
	}else if(premioSeleccionado.gameId==1121){//KN
		openPreviewWindowPP(premioSeleccionado.gameId,"Kinelo",premioSeleccionado.ticketId);
	}
}
...
function desactivarPantallaRecurrenteTransferenciaPML(){
	pantallaListaRecurrentePML=false;
...
function activarPantallaRecurrenteTransferenciaPML(){
	pantallaListaRecurrentePML=true;
...
function reenviarRetiroTransferenciaPinSms(){   
	console.log("reenviarRetiroTransferenciaPinSms");
	$('#ecoPinMensajeError').html("");
    $("#code-01").val("");
    $("#code-02").val("");
    $("#code-03").val("");

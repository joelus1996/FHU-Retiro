 ...
 */
@Repository
public class DrawDaoImpl extends HibernateBaseDaoImpl  implements DrawDao {
	...
	public Draw findByIdByGameId(Integer drawId, Integer gameID) throws Exception {
		...
		try {	 	
	        ...
	        objectDomain = (Draw)DataAccessUtils.uniqueResult(resultQuery);	        	        
		} catch(Exception e) {	
			LoggerApi.severe(e);
			throw e;
		} finally {
			if (objectDomain != null) {
				LoggerApi.Log.info("DR_ADDITIONAL= "+objectDomain.getDrAdditional()+
								   ...
								   " DR_ADDON_RESULT_2= "+objectDomain.getDrAddonResult2()+
								   " DR_ADDON_RESULT_1= "+objectDomain.getDrAddonResult1());
			}
	}
		return objectDomain;
}
	...
	public List<Draw> findByDrawListByGameId(Integer gameId, Integer days) throws Exception {
		...
		LoggerApi.Log.info("gameId=" + gameId + "days=" + days);		
		List<Draw> resultQueryList = new ArrayList<Draw>();
		try {
			Object[] values = new Object[2];
	        values[0] = gameId;
	        values[1] = days;
	        String queryString = "FROM Draw d " +
	        					 "WHERE d.id.gameId = ? and " +
	        					 	   "d.drResult is not null and   " +
	        					 	   "d.drDate >= sysdate - ? " +
	        					  "ORDER BY d.id.drawId DESC"; 

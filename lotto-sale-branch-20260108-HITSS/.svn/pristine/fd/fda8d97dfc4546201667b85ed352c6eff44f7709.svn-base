...
var $kabala= function () {
  ...
  //Plus+Chau Chamba
    ...
    var chChDrawEnabled = $("#chChDrawEnabled").val();
    ...
    $(".clear").click(function () {
        if (status == "ACT") {
        	...
        	if(dataLayerlimpiar){
        		datalayerMarcarBolillasCotejar('Limpiar','Marcar 6 bolillas o más');
        	}
       ...
       if (status == "ACT") {
        ...
        //    }
            if (total > 6006) {
                ...
                datalayerErroresIframe('Cotejar','Paso 2','Cotejar','El total de apuestas no debe ser mayor a 6006');
            }
        }
		    var fecha_ini_1=$("#mySelectBox").val();
			var from=$("#mySelectBox-2").val();
			var to=parseInt(fecha_ini_1)+parseInt(from);
			    var $cantNumBarra = $("#kabala_cotejado_codigoBarras").children(".BotonCotejo");
                var codigoBarras="";
			if(direc=="barra"){
					$.each($cantNumBarra, function(value){					
						codigoBarras += $($cantNumBarra).eq(value).val() +" ";
					})
			}
			//alert("entrastes");
		...
		//alert("content_object: "+content_object);
			...
			//alert("entro222");
                ...
                result_ticket=result_ticket+"/"+from+"/"+to+"/"+codigoBarras;
                $("#ajax-loader").show();
				//alert("entro1");
				var result="";
                var resultJson="";
				var contador = 0;
				//alert("entro1111");
                $.ajax({type: "POST", url: "cotejo_ajax.html", dataType: "text", data: "dato=" + result_ticket + "&tip="+direc ,async:false, success: function (e) {
				...
				$.map(resultJson, function (d) {
					...
					if (d.message == 'OK') {
						...
						$.map(d.datos_jugada_A, function (f) {
							//alert(f);
							a += header_auxiliar+'<div class="top-single-game"><div class="row container"><div class="col-6"><h3>NRO DE SORTEO:<span>' + f.sorteo + '</span></h3></div><div class="col-6"><h3>FECHA: <span>' + f.fecha + '</span></h3></div></div></div>';							
							a += '<div class="top-single-game"><div class="row container"><div class="col-12"><h3>RESULTADO DE SORTEO POZO BUENAZO:<span>' + f.resultado + '</span></h3></div></div></div>';					
			                
			                if(f.resultadoChauchamba!="null" && f.resultadoChauchamba!=null){			                    
			                    a += '<div class="top-single-game"><div class="row container"><div class="col-12"><h3>RESULTADO DE SORTEO CHAU CHAMBA:<span>' + f.resultadoChauchamba + '</span></h3></div></div></div>';
			                }                
			                
							
							var o=0;l=0;m=0;q=0;
							if(f.jugadaA!=null){
								a += '<div class="body-single-game"><div class="body-game"><div class="button-group checkboxes-ball clearfix"><h3>JUGADA A</h3><span>' + f.jugadaA + '</span></div></div></div>';
		                        a += '<div class="kabala_jugada_acerts">';
								 $.map(f.cotejo_jugada_A, function (c) {
				                    ...
				                    //a += '<div Style="margin-top: -10px;"><span class="indicador_cotejo"><b>[' + q + ']</b>  <span class="cotejador">' + c + '</span></span></div></br>'									
									 a += '<div class="row container"><span><span class="cotejador">' + c + '</span></span></div>';
				                });
								 a += '</div>';
							}
							
							if(f.jugadaB!=null){
		        				a += '<div class="body-single-game"><div class="body-game"><div class="button-group checkboxes-ball clearfix"><h3>JUGADA B</h3><span>' + f.jugadaB + '</span></div></div></div>';
		                        a += '<div class="kabala_jugada_acerts">';

		                         //addmark jugada B
		        				 $.map(f.cotejo_jugada_B, function (s) {
		                            //o++;
		                            a += '<div class="row container"><span><span class="cotejador">' + s + '</span></span></div>';
		                         });
		                        a += '</div>';
							...
							if(f.jugadaC!=null){
		        				a += '<div class="body-single-game"><div class="body-game"><div class="button-group checkboxes-ball clearfix"><h3>JUGADA C</h3><span>' + f.jugadaC + '</span></div></div></div>';
		                        a += '<div class="kabala_jugada_acerts">';
		        	
		                         //addmark jugada C
		                         $.map(f.cotejo_jugada_C, function (t) {
		                            //l++;
		                            a += '<div class="row container"><span><span class="cotejador">' + t + '</span></span></div>';
		                         });
							...
							if(f.jugadaD!=null){
		        				a += '<div class="body-single-game"><div class="body-game"><div class="button-group checkboxes-ball clearfix"><h3>JUGADA D</h3><span>' + f.jugadaD + '</span></div></div></div>';
		                        a += '<div class="kabala_jugada_acerts">';
		                         ...
		                         $.map(f.cotejo_jugada_D, function (h) {
		                            //m++;
		                            a += '<div class="row container"><span><span class="cotejador">' + h + '</span></span></div>';
		                         });
							...
							if(f.premios!=null){
								a += '<div class="body-single-game"><div class="body-game"><div class="button-group checkboxes-ball clearfix"><h3>Premios obtenidos</h3><span>' + f.premios + '</span></div></div></div>';								
							}
		                ...
		                a += '<span class="datos_enunciado_cotejo_right">' + d.message + '</span></br>';
		                a += '<span class="datos_enunciado_cotejo_right">Revise los datos ingresados</span></br>';
		                a += '<div class="datos_separador"><span>..........................................................................................................................................</span></div>';
					}
					
				});
				$("#valorJugada").html(a);
				$('.footer-single-game').show();
            } 
	...
	function cotejoCodigoBarra(){
		jAlert('Estamos trabajando en esta opción, coteje su jugada marcando los numeros', null);
		$.ajax({type: "POST", url: "cotejo_ajax.html", dataType: "text", data: "dato=" +"5218244548712458751"+"&tip="+"barra",async:false, success: function (e) {
			resultJson= $.parseJSON(e);	
			result=e;
			}})
	}
    ...
    function buy() {
        ...
        if((0<value_numberJ1.length && value_numberJ1.length<6) || (0<value_numberJ2.length && value_numberJ2.length<6) || (0<value_numberJ3.length && value_numberJ3.length<6) || (0<value_numberJ4.length && value_numberJ4.length<6)){
        	...
        	try {
        		cotejaBoletoError("Kabala", label);	
			} catch (e) {
				console.error(e);
			}
			datalayerErroresIframe('Cotejar','Paso 2','Cotejar',label);
            jAlert(label, null);                     
        } else if (total<1 && costoTotalBet<=0) {    	   
    	   if(foco == false){
    		  var codigo = $("#kabala_cotejado_codigoBarras").children("input");
			  for(i =0 ; i < codigo.length-1 ; i++){
				 codigoBarra += ""+codigo.eq(i).val()
			  }    		  
			  if (codigoBarra.length < 30) {
				  label='Complete el codigo de barra para cotejar su jugada';
				  try {
		        		cotejaBoletoError("Kabala", label);	
					} catch (e) {
						console.error(e);
    ...
    $('#btn_desktop_cotejar_boleto_kabala').on('click',function () {
       /* if(status == 'ACT'){ 
    ...
    function combinatoria() {
        ...
        //alert("actdraw="+actdraw+" sorteos="+sorteos+" draw="+draw);
        if(parseFloat(actdraw)>=parseFloat(chChDrawEnabled)) {
        	active_chch();
        } else {
        	active_plus();
        	active_chauchamba();
    ...
    function active_chauchamba() {
        ...
        add_ChauCmb3=0;
        add_ChauCmb4=0;    
    	if($('#J1-check-plus').prop('checked')) $('#J5-check-chauchamba').prop("disabled",false);
		else $('#J5-check-chauchamba').prop('checked', false).prop('disabled', true);
        if($('#J2-check-plus').prop('checked')) $('#J6-check-chauchamba').prop("disabled",false);
		else $('#J6-check-chauchamba').prop('checked', false).prop('disabled', true);
        if($('#J3-check-plus').prop('checked')) $('#J7-check-chauchamba').prop("disabled",false);
		else $('#J7-check-chauchamba').prop('checked', false).prop('disabled', true);
        if($('#J4-check-plus').prop('checked')) $('#J8-check-chauchamba').prop("disabled",false);
		else $('#J8-check-chauchamba').prop('checked', false).prop('disabled', true);
		if($('#J5-check-chauchamba').is(':checked')) add_ChauCmb1=1;
		else add_ChauCmb1=0;
		if($('#J6-check-chauchamba').is(':checked')) add_ChauCmb2=1;
		else add_ChauCmb2=0;
		if($('#J7-check-chauchamba').is(':checked')) add_ChauCmb3=1;
		else add_ChauCmb3=0;
		if($('#J8-check-chauchamba').is(':checked')) add_ChauCmb4=1;
		else add_ChauCmb4=0;
    }
    ...
    function active_chch() {
        ...
        add_ChauCmb3=0;
        add_ChauCmb4=0;
        //alert("cantin_1="+cantin_1+" cantin_2="+cantin_2+" cantin_3="+cantin_3+" cantin_4="+cantin_4);
        if(cantin_1 > 0) $('#J1-check-chauchamba').prop("disabled", false);
        else $('#J1-check-chauchamba').prop('checked', false).prop("disabled", true);
        ...
        if($('#J1-check-chauchamba').is(':checked')) add_ChauCmb1=1;
		else add_ChauCmb1=0;
		if($('#J2-check-chauchamba').is(':checked')) add_ChauCmb2=1;
		else add_ChauCmb2=0;
		if($('#J3-check-chauchamba').is(':checked')) add_ChauCmb3=1;
		else add_ChauCmb3=0;
		if($('#J4-check-chauchamba').is(':checked')) add_ChauCmb4=1;
		else add_ChauCmb4=0;
    }
    ...
    $("#more_plays").click(function () {
        ...
        $(".flecha_next").show();
        $(".flecha_back").hide();
        $('#mySelectBox-2').val($('#mySelectBox-2').find('option').eq(0).attr('value'));
        $('#mySelectBox').val($('#mySelectBox').find('option').eq(0).attr('value'));
        $('#mySelectBox').closest('.selectBox').find('.box').html($('#mySelectBox').val());
        $('#mySelectBox-2').closest('.selectBox').find('.box').html($('#mySelectBox-2').find('option').eq(0).text());
        dataLayerlimpiar = false;
   ...
   $("#btn_finaliza_compra").click(function () {
    ...
    function changeMySelectBox(){
    	//alert("ENTRO "+parseFloat($("#mySelectBox-2").val())+"-"+parseFloat(chChDrawEnabled));
    	if(parseFloat($("#mySelectBox-2").val())>=parseFloat(chChDrawEnabled)) {
    		$("#increases-plus-chch1").addClass("disable");
    		$("#increases-chch1").removeClass("disable");
    		$("#increases-plus-chch2").addClass("disable");
    		$("#increases-chch2").removeClass("disable");
    		$("#increases-plus-chch3").addClass("disable");
    		$("#increases-chch3").removeClass("disable");
    		$("#increases-plus-chch4").addClass("disable");
    		$("#increases-chch4").removeClass("disable");
    		$('#J1-check-chauchamba').prop('checked', false);
    		$('#J2-check-chauchamba').prop('checked', false);
    		$('#J3-check-chauchamba').prop('checked', false);
    		$('#J4-check-chauchamba').prop('checked', false);
    		if(cantin_1 > 0) $('#J1-check-chauchamba').prop("disabled", false);
            else $('#J1-check-chauchamba').prop("disabled", true);
            if(cantin_2 > 0) $('#J2-check-chauchamba').prop("disabled", false);
            else $('#J2-check-chauchamba').prop("disabled", true);
            if(cantin_3 > 0) $('#J3-check-chauchamba').prop("disabled", false);
            else $('#J3-check-chauchamba').prop("disabled", true);
            if(cantin_4 > 0) $('#J4-check-chauchamba').prop("disabled", false);
            else $('#J4-check-chauchamba').prop("disabled", true);
    	} else {
    		$("#increases-plus-chch1").removeClass("disable");
    		$("#increases-chch1").addClass("disable");
    		$("#increases-plus-chch2").removeClass("disable");
    		$("#increases-chch2").addClass("disable");
    		$("#increases-plus-chch3").removeClass("disable");
    		$("#increases-chch3").addClass("disable");
    		$("#increases-plus-chch4").removeClass("disable");
    		$("#increases-chch4").addClass("disable");
    		$('#J1-check-plus').prop('checked', false);
    		$('#J2-check-plus').prop('checked', false);
    		$('#J3-check-plus').prop('checked', false);
    		$('#J4-check-plus').prop('checked', false);
    		if(cantin_1>0) $('#J1-check-plus').prop("disabled", false);
            else $('#J1-check-plus').prop("disabled", true);
            if(cantin_2>0) $('#J2-check-plus').prop("disabled",false);    
            else $('#J2-check-plus').prop("disabled", true);
            if(cantin_3>0) $('#J3-check-plus').prop("disabled",false);    
            else $('#J3-check-plus').prop("disabled", true);
            if(cantin_4>0) $('#J4-check-plus').prop("disabled",false);     			
            else $('#J4-check-plus').prop("disabled", true);
            $('#J5-check-chauchamba').prop('checked', false);
    		$('#J6-check-chauchamba').prop('checked', false);
    		$('#J7-check-chauchamba').prop('checked', false);
    		$('#J8-check-chauchamba').prop('checked', false);
            $('#J5-check-chauchamba').prop('disabled', true);
            $('#J6-check-chauchamba').prop('disabled', true);
            $('#J7-check-chauchamba').prop('disabled', true);
            $('#J8-check-chauchamba').prop('disabled', true);
    ...
    $("#mySelectBox-2").on("change",function(){
    	changeMySelectBox();
    })
};
$($kabala);
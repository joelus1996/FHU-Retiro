  ...
  //Plus+Chau Chamba
    ...
    $(".clear").click(function () {
        if (status == "ACT") {
        	...
        	if(dataLayerlimpiar){
        		datalayerMarcarBolillasCotejar('Limpiar','Marcar 5 bolillas o más');
        	}
	...
	var run_cotejo = {toJSON: function (data) {
		...
		//alert("content_object: "+content_object);
			...
			//alert("entro222");
				...
				var contador = 0;
				//alert("entro1111");
                $.ajax({type: "POST", url: "cotejo_ganadiario_ajax.html", dataType: "text", data: "dato=" + result_ticket,async:false, success: function (e) {
				...
				//alert(f);
                    ...
                    a += '<div class="top-single-game"><div class="row container"><div class="col-12"><h3>RESULTADO DE SORTEO:<span>' + f.resultado + '</span></h3></div></div></div>';
				...
				if(f.jugadaA!=null){
                    ...
                    a += '<div class="kabala_jugada_acerts">';
    				 ...
    				 $.map(f.cotejo_jugada_A, function (c) {
                        q++;
                        a += '<div class="row container"><span><span class="kabala_jugada-title">[' + q + ']</span> <span class="cotejador">' + c + '</span></span></div>';
                     });
				...
				if(f.jugadaB!=null){
                    ...
                    a += '<div class="kabala_jugada_acerts">';
    				 ...
    				 $.map(f.cotejo_jugada_B, function (s) {
                        o++;
                        a += '<div class="row container"><span><span class="kabala_jugada-title">[' + o + ']</span> <span class="cotejador">' + s + '</span></span></div>';
                     });
                    a += '</div>';
                    ...
                    a += '<div class="kabala_jugada_acerts">';
                     ...
                     $.map(f.cotejo_jugada_C, function (t) {
                        l++;
                        a += '<div class="row container"><span><span class="kabala_jugada-title">[' + l + ']</span> <span class="cotejador">' + t + '</span></span></div>';
                     });
                    ...
                    a += '<div class="kabala_jugada_acerts">';
                     ...
                     $.map(f.cotejo_jugada_D, function (h) {
                        m++;
                        a += '<div class="row container"><span><span class="kabala_jugada-title">[' + m + ']</span> <span class="cotejador">' + h + '</span></span></div>';
                     });
    ...
    function buy() {
        ...
        if((0<value_numberJ1.length && value_numberJ1.length<5) || (0<value_numberJ2.length && value_numberJ2.length<5) || (0<value_numberJ3.length && value_numberJ3.length<5) || (0<value_numberJ4.length && value_numberJ4.length<5)){
        	...
        	try {
 		   			cotejaBoletoError("Gana Diario", label);				
    			} catch (e) {
    				console.error(e);
    			}
    ...
    $('#btn_desktop_cotejar_boleto_ganadiario').on('click',function () {
        if(status == 'ACT'){ 
...
//            }
    ...
    function combinatoria() {
        ...
        if (total >= 1) {
            ...
            if (algorithm == "BETS") {
                ...
                costoTotalBet = callTransformByBets(total, sorteos, v_data_value_bet, bets, pay) + (sorteos);
            } else {
                if (algorithm == "COST") {
                    costoTotalBetMensaje = callTransformByCost(parseInt(total) + 1, sorteos, v_data_value_bet, bets, cost);
                    costoTotalBet = callTransformByCost(total, sorteos, v_data_value_bet, bets, cost) + (sorteos);
                } else {
                    if (algorithm == "DRAW") {
                        costoTotalBetMensaje = callTransformByDraws(total, parseInt(sorteos) + 1, v_data_value_bet, draw, pay);
                        costoTotalBet = callTransformByDraws(total, sorteos, v_data_value_bet, draw, pay) + (sorteos);
                    } else {
    ...
    $('.left-panel').on('click','.process-delete1',function(){
        ...
        var costo_total = 0;
        for (var i in content_object) {
            costo_total += content_object[i][5]
        }
        $(".result1").html("S/." + floatFormat(costo_total));
        if(content_object.length==0){
            $('#more_plays').click();
    ...
    $('.left-panel').on('click','.process-delete2',function(){
        ...
        for (var i in array_no_procesados) {
            costo_total += parseFloat(array_no_procesados[i][5])
        }
        $(".result1").html("S/." + floatFormat(costo_total));
   ...
   $("#btn_finaliza_compra").click(function () {
    ...
    var a = '';
    var i = 0, j = 0, h = 0, m = 0;
    if ((bet1 > 2) && (bet2 < 5) && (bet3 < 5) && (bet4 < 5)) {
        $.map(w, function (e) {
    ...
    } else if ((bet1 > 2) && (bet2 > 5) && (bet3 < 5) && (bet4 < 5)) {
        $.map(w, function (e) {

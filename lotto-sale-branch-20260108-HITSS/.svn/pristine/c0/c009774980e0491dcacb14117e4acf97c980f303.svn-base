...
public class ApiClient {
    ...
    private static final List<Integer> listHttpStatusAllowed = Arrays.asList(
            ...
            HttpURLConnection.HTTP_FORBIDDEN, ApiClient.HTTP_UNPROCESSABLE_ENTITY 
    );
    ...
    try {    
        ...
        connection.getOutputStream().write(body.getBytes(Constants.CHARSET_UTF8));
        connection.setConnectTimeout(Integer.parseInt(ConnectionFactory.operationProperty("connectTimeout", Constants.contextSale )));
        connection.setReadTimeout(Integer.parseInt(ConnectionFactory.operationProperty("readTimeout", Constants.contextSale)));
        ...
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream, Constants.CHARSET_UTF8));

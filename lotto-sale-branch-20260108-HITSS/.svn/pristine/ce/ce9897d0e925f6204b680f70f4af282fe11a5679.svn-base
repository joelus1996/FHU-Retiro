...
public class JackpotCasinoUtils{
	...
	public static void updateJackpotCasino() throws Exception{
		...
		if ( !ConnectionFactory.operationProperty("activeUpdateJackpot", Constants.contextSale).equals("true") ) return;
		try {			
			int timeRetry = 1;
	...
	private static String updateJackpotTrx(int times, String log) throws Exception { 
		...
		try { 
        	...
        	surl= Constants.URL_JACKPOT_CASINO;
            ...
            if (conn.getResponseCode() == 200) {
            	...
            	if(root.toString().length() > 2) {   				
					for(JsonElement je1 : root.getAsJsonArray()) {
						...
						Constants.jackpotsMap.put(removeUnfit(jObject1.get("jackpotid").toString()), formatoJackpot(removeUnfit(jObject1.get("valor").toString())) );
					} 
            	} 	
			}else {
			...
			//LoggerApi.Log.info(log+" end "+times+" [updateJackpotTrx] response valJackpot="+valJackpot+" in "+(System.currentTimeMillis()-startTime)/1000.0 +" seconds");
	...
	private static String removeUnfit(String str) {		
		return str.replaceAll(Constants.CASINO_REGEX, Constants.CASINO_VACIO);
	}

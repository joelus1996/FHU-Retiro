...
@Repository
public class FechazaSaleDaoImpl extends HibernateBaseDaoImpl implements
		FechazaSaleDao {

	@Override
	public FechazaProcedureBetData findBetData(Integer p_clientId)
			throws Exception {
		LoggerApi.Log.info("clientId= " + p_clientId);

		List<FechazaProcedureBetData> resultQuery = new ArrayList<FechazaProcedureBetData>();
		...
		try {
			...
			values[0] = p_clientId;
			resultQuery = super.findForNamed("FECHAZASALE_BETDATA", values);
			objectDomain = (FechazaProcedureBetData) DataAccessUtils.uniqueResult(resultQuery);
	...
	public List<FechazaProcedureDrawData> findListDrawData() throws Exception {
		...
		try {
			...
			resultQueryList = super.findForNamed("FECHAZASALE_DRAWDATA", values);

package pe.com.intralot.loto.layer.controller.game.clicygana;
...
import pe.com.intralot.loto.layer.service.game.clicygana.bo.ClicyganaBo;
import pe.com.intralot.loto.model.Game;
import pe.com.intralot.loto.sale.lib.LoggerApi;
import pe.com.intralot.loto.util.Constants;

@Controller
public class ClicyganaController {

    @Autowired
    private ClientSaleBo clientSaleBo;
    @Autowired
    private ClicyganaBo clicyganaBo;

    @RequestMapping(value = "/juega-clicygana")
    public String findByIdCliente(HttpServletRequest request, HttpServletResponse response, ModelMap modelList) throws Exception {
        ...
        try {
            ...
            String idGameParameter = request.getParameter("id");
            String idTipGameParameter = request.getParameter("tip");
            if (idGameParameter == null)
                idGameParameter = "0";
            request.setAttribute("idGameParameter", idGameParameter);
            request.setAttribute("idTipGameParameter", idTipGameParameter);
            System.out.print("parametro :" + idGameParameter);
            ClicyganaProcedureBetData clicyganaSaleBean = clicyganaBo.findByClientId(idClient);
            request.setAttribute("clicyganaSale", clicyganaSaleBean);
            userBean.setpGame(Game.CLICYGANA);
            session.setAttribute(Constants.USR_SESION, userBean);
            /*if(idTipGameParameter == "1")
            	return "game/clicygana/home_clicygana_user";
            else if(idTipGameParameter == "0")
            	 return "game/clicygana/play_clicygana_user";
            else*/
            return "game/clicygana/home_clicygana_user";
        } catch (Exception e) {
            LoggerApi.severe(e);
            ...
            request.setAttribute("isAllowed", clicyganaBo.isAllowedGoIn(user));
        }
    }

    /*@RequestMapping(value = "/login_clicygana")
    public void loginByUserPass(HttpServletRequest request, HttpServletResponse response, ModelMap objectModelMap) throws Exception {
        ...
        try {
        	if(request.getParameter("captcha-client") == null) {
        		LoggerApi.Log.info("Entro a ClicyganaController loginByUserPass sin captcha");
        		captcha = 1;
			} else {
				...
				if(capchaCode.equals(sessionCaptchaCode)) {
					Logger.getLogger(LoggerAPI.SALECARD).info("Entro a ClicyganaController loginByUserPass con captcha");
					captcha = 2;
				}
			}
                    ...
                    if (clientProcedureLogin != null) {
                        ...
                        if (state == 1) {
                            ...
                            if (mode == 0) {
                                ...
                                ClicyganaProcedureBetData clicyganaProcedureBetData = clicyganaBo.findByClientId(idClient);
                                Double monto1 = clicyganaProcedureBetData.getBalanceAmount();
                                Double monto2 = clicyganaProcedureBetData.getBalanceAmountGame();
                                String user = clicyganaProcedureBetData.getUser();
                                String nickName = clicyganaProcedureBetData.getNickName();
    ...
    @RequestMapping(value = "/play_clicygana_user")
    public ModelAndView findPrizeTableClicyGana(HttpServletRequest request, HttpServletResponse response, ModelMap objectModelMap) throws Exception {
        String sessionCode = request.getParameter("sessionCode");
        String user = request.getParameter("user");
        String mode = request.getParameter("mode");
        String nickName = request.getParameter("nickName");
        String gameId = request.getParameter("gameId");
        String idGameParameter = request.getParameter("idGameParameter");
        String idTipGameParameter = request.getParameter("idTipGameParameter");
        try {
            ...
            request.setAttribute("idTipGameParameter", idTipGameParameter);
            return new ModelAndView("game/clicygana/play_clicygana_user");
        } catch (Exception e) {
            LoggerApi.severe(e);
            return new ModelAndView("game/clicygana/play_clicygana_user");
        } finally {
    ...
    @RequestMapping(value = "/command_cashier")
    public void findCommandCashier(HttpServletRequest request, HttpServletResponse response, ModelMap objectModelMap) throws Exception {
        String sessionCode = request.getParameter("sessionCode");
        try {
            PrintWriter out = null;
            out = response.getWriter();
            ClicyganaProcedureCommandCashier clicyganaProcedureCommandCashier = clicyganaBo.findCommandCashier(sessionCode);
            out.print(clicyganaProcedureCommandCashier.getMessage());
        } catch (Exception e) {
            LoggerApi.severe(e);
            throw e;
        } finally {
    ...
    @RequestMapping(value = "/command_close")
    public void findCommandClose(HttpServletRequest request, HttpServletResponse response, ModelMap objectModelMap) throws Exception {
        String sessionCode = request.getParameter("sessionCode");
        try {
            PrintWriter out = null;
            out = response.getWriter();
            ClicyganaProcedureCommandClose clicyganaProcedureCommandClose = clicyganaBo.findCommandClose(sessionCode);
            out.print(clicyganaProcedureCommandClose.getMessage());
        } catch (Exception e) {
            LoggerApi.severe(e);
            throw e;
        } finally {

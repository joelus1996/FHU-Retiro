...
import pe.com.intralot.loto.layer.model.domain.TYCPDPProcedureConsultPendingDocuments;
import pe.com.intralot.loto.layer.model.domain.TYCPDPProcedureConsultPendingDocumentsTemplate;
...
//				session.setAttribute("openSession","openSession");
	...
	public ResultBean putSmsClient(Integer p_clientId, String number, ClientSaleBo clientSaleBo) {
    	...
    	try {
    		...
    		if(result!=null && result.getCode() !=null && result.getCode()==200 && result.getStatus()==1) {
    			...
    			message = "El c&oacute;digo de activaci&oacute;n no ha sido procesado. Intenta ingresar a tu cuenta m&aacute;s tarde.";//"Error al enviar el c&oacute;digo, int&eacute;ntelo m&aacute;s tarde.";  	
    		}
        } catch (Exception e) {
	...
	public ResultBean putWaClient(Integer p_clientId, String number, ClientSaleBo clientSaleBo) {
    	ResultBean bean = new ResultBean();
		String message = "Server Error [500]";
		Integer status = 500;
		String randomCode = "";
...
//        	out.print(o);
	...
	public String securitySendAlertMail(String email, String date, Integer client_id, String ip) throws Exception {
        ...
        try {
        	...
        	result = MailLib.sendValidMailEco_Security(ConnectionFactory.operationProperty("mailSourceApp", Constants.contextSale),mailSender, mailSubject, mailBody,context.getRealPath(Constants.INTERNAL_PATH_LOGO_MAIL), context.getRealPath(Constants.INTERNAL_PATH_LOGOS_JUEGOS_MAIL_V2),ConnectionFactory.operationProperty("servidorCorreoProteccionUsuario", Constants.contextSale) );
        } catch (Exception e) {
            LoggerApi.severe(e);
            result = e.getMessage();
        } finally {
	...
	public String validationreniecWs(String log, String json) throws Exception { 
		...
		try {
			...
			String userReniecAPI = Constants.USER_RENIEC_WS;
			String passReniecAPI = Constants.PASS_RENIEC_WS;
			String credenciales = userReniecAPI+":"+passReniecAPI;
			credenciales = Base64.encodeBase64String(credenciales.getBytes());
            
			
			URL url = new URL(Constants.API_RENIECWS);
			...
			if(statusCode < HttpServletResponse.SC_BAD_REQUEST) {
				reader = new BufferedReader(new InputStreamReader(conn.getInputStream(),Constants.CHARSET_UTF8));
			}else {
				LoggerApi.Log.info("HTTP CODE: " + statusCode + "JSON: " + json);
				reader = new BufferedReader(new InputStreamReader(conn.getErrorStream(),Constants.CHARSET_UTF8));
			}

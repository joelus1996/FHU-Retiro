...
public class VideoLoteriaUtils extends Thread{
	...
	public String getLobbyUrlVl(String external_id, String game_id, String return_url, String language) {
    	...
    	try {
    		URL url = new URL(Constants.VIDEO_LOTERIA_API_URL+"get/lobby");
    		HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constants.VIDEO_LOTERIA_API_URL.startsWith("https")) {
	            try {
	                SSLContext sc;
	        ...
	        params.put("site_id", Constants.VIDEO_LOTERIA_SITE_ID);
	    	params.put("access_key", Constants.VIDEO_LOTERIA_ACCESS_KEY);	
	    	params.put("language", Constants.VIDEO_LOTERIA_LANGUAGE);
	    	params.put("username", external_id);
	    	params.put("currency", Constants.VIDEO_LOTERIA_CURRENCY);
	...
	public String createPlayer(String external_id) {
    	...
    	try {
    		...
    		URL url = new URL(Constants.VIDEO_LOTERIA_API_URL+"player/create");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constants.VIDEO_LOTERIA_API_URL.startsWith("https")) {
	            try {
	...
	public String checkPlayer(String external_id) {
	    ...
	    try {
	        URL url = new URL(Constants.VIDEO_LOTERIA_API_URL + "player/check");
	        ...
	        writer.append(LINE_FEED).append(Constants.VIDEO_LOTERIA_SITE_ID).append(LINE_FEED).flush();

	        // access_key
	        writer.append("--" + boundary).append(LINE_FEED);
	        writer.append("Content-Disposition: form-data; name=\"access_key\"").append(LINE_FEED);
	        writer.append("Content-Type: text/plain; charset=" + charset).append(LINE_FEED);
	        writer.append(LINE_FEED).append(Constants.VIDEO_LOTERIA_ACCESS_KEY).append(LINE_FEED).flush();

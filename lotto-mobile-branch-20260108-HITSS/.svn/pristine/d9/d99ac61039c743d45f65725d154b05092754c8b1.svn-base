...
import pe.com.intralot.loto.layer.controller.client.boimpl.ClientLotocardBoImpl;
...
public class TokenSessionFilter implements Filter {	
	...
	@Autowired
    private ClientLotocardBoImpl clientBo;
...
//        System.out.println("URL que pasa por el filtro: "+url);
        ...
        if ((ClientProcedureGetLoginData) session.getAttribute(Constantes.USR_SESION) != null && ((ClientProcedureGetLoginData) session.getAttribute(Constantes.USR_SESION)).getSessionId() != null
                && ((ClientProcedureGetLoginData) session.getAttribute(Constantes.USR_SESION)).getClientId() != null && !suburl.equals("validate-session.html")) {
...
//	        		httpResponse.sendRedirect("sessionexpirada.html");
...
////		        		System.out.println("Cierre de session por validateSession: Tiempo de inactividad: "+(tiempoInactivo/1000));
//		            }
//	//	            	chain.doFilter(request, response);
//		            
//				} catch (Exception e) {

...
<meta name='description' content='Kinelo, resultados de sorteos anteriores' />
   ...
   <BR>
    ...
    <ul style="margin-right: 5px; margin-left: 5px;" class="listContent listContentCuadro">	
			...
			<c:forEach begin="${rowBEGIN-1}" step="1" end="${rowEnd-1}"
				var="item" varStatus="status" items="${resultGamesKinelo}">
			...
			<c:set value="${fn:length(resultGamesKinelo)}" var="rLen" />
       ...
       <c:forEach items="${dateResultKinelo}" var="item"> 
       
      
	    <a href="#" onclick="window.location.href='game_kinelo_filter_result.html?filter=${fn:substring(item,0,10)}';" 

...
class AlertDialog {
  ...
  addEventListeners() {
    this.closeEl.addEventListener('click', this.defaultFunction);

    this.btnAgreeEl.addEventListener('click', this.defaultFunction);

    this.btnDisagreeEl.addEventListener('click', this.defaultFunction);
  }

  open(config, iframe = null) {
    this.configure(config, iframe);
  ...
  close() {
    ...
    if (this.checkPayment) {
      this.btnAgreeEl.removeEventListener('click', this.checkPayment);
    }
  }
  ...
  configure(config, iframe = null) {
    ...
    if (this.btnAgreeEl && agree) {
      ...
      if (agree.type === 'check-payment') {
        this.btnAgreeEl.removeEventListener('click', this.defaultFunction);

        this.checkPayment = (e) => {
          iframe.iFrameResizer.sendMessage(
            { type: 'check-payment', data: {} },
            '*'
          );
          this.close();
          e.preventDefault();
        ...
        this.btnAgreeEl.addEventListener('click', this.checkPayment);
      } else if (agree.type === 'redirect' && agree.url) {
          this.btnAgreeEl.addEventListener('click', (e) => {
            window.location.href = agree.url
            this.close()
            e.preventDefault()
            e.stopPropagation()
          })
  ...
  window.iFrameResize(
    {
      ...
      onMessage: ({ iframe, message }) => {
        ...
        // Open alert dialog
        if (type === 'open-alert') {
          // necesito de iframe para mandar mensajes
          $alertDialog.open(data, iframe);
        }
         ...
         * Only for mobile
         */
        if (type === 'back-to-play-page') {
          window.location.href =
            'game_tinka_show_shoppingcart_tinkaexpress.html';
        }
        ...
        if (type === 'payment-complete') {
          ...
          $.ajax({
            type: 'POST',
            url: 'game-tinka-delete-jugada.html',
            dataType: 'json',
            data: {},
            success: function () {

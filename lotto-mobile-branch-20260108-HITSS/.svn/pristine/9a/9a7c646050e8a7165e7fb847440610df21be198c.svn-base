...
<body class="main-tinka" onload="deshabilitaRetroceso()">
	<!-- Google Tag Manager (noscript) -->
	<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-58FNN4L"
	height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
	<!-- End Google Tag Manager (noscript) -->
    <input type="hidden" id="jugadasActuales" value="${jugadasActuales}"/>
	<input type="hidden" id="precioFin" value="${formatDiscountPayment2}"/>
	<input type="hidden" id="draws" value="${draws}"/>
									...
									<c:forEach 	var="item"  items="${boleto}">
											...
											<div class="my-game">
												...
												<div class="game-options">
													<a href="#" class="game-option ico ico-close" onclick="datalayerRemover();taggingCombosTinkerosRemove();window.location.href='game_tinka_delete_bet.html?id=${item.key}';" >
									...
									<div class="centrar_preciosf">
										<ul>
											<li>
												...
												<span class="td_f">${formatRegularPayment}</span>
											</li>
											<li>
												<span class="td_r">Total de jugadas:</span>
												<span class="td_f">${jugadasActuales}</span>
											</li>
											<li>
												<span class="td_r">Descuento:</span>
												<span class="td_f">${discount}</span>
											</li>
										</ul>
										...
										<ul>
											<li>
												...
												<span class="td_f">${formatDiscountPayment}</span>
											</li>
										</ul>
								...
								<div class="game-actions">
									...
									<c:if test="${!empty clientId && agreement!=''}">
										<c:if test="${indicadorPurchase==negacion}">
											<button type="button" id="btn_mobile_comprar_combo_tinka" class="btn btn-red float-bottom">COMPRAR</button>
										</c:if>
									
										<c:if test="${indicadorPurchase==afirmacion}">
											<form action="game_tinka_play_bet_result_suscription.html" method="post" id="game_tinka_play_bet_result_suscription_s">
												<input id="play" name="boleto" type="hidden" value="boleto_${draws}">								
												<button type="button" id="btn_mobile_comprar_combo_tinka_s" class="btn btn-red float-bottom">COMPRAR</button>
											</form>
										</c:if>
									</c:if>
									...
									<c:if test="${empty clientId or agreement==''}">
										...
										<c:if test="${indicadorPurchase==afirmacion}">
											<form action="security_login_execute_authentication.html">
												...
												<input  name="display" type="hidden" value="tinkaSuscripcion">
												<button type="button" id="btn_mobile_comprar_combo_tinka" class="btn btn-red float-bottom">COMPRAR</button>
											</form>
										</c:if>
									</c:if>
								...
								<div id="popup_terms" class="overlay">
									<div class="popup popup-sm"  style="height: 95%;">
										...
										<div class="wrap-modal">
											<jsp:include page="../../client/interface-term-tinka-subscription.jsp"/>
										</div>
									</div>
								</div>
							</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(draws!=undefined && draws!=null && draws!=''){ 
				var name="Tinka Combo "+draws+" jugadas"
				var id="tinkaC"+draws;
				var price =$("#precioFin").val();	
				datalayerAddToCart(jugadas,importPagar/jugadasActuales,'Elige 6 números o más');
				taggingCombosTinkerosFinaliza(draws);
				taggingCombosTinkerosEEcheckout(name,id,price);	
			}
		});
		...
		function taggingCombosTinkerosRemove(){
			try {
				var draws =$("#draws").val();
				if(draws!=undefined && draws!=null && draws!=''){ 
					var name="Tinka Combo "+draws+" jugadas"
					...
					console.log("Tagging taggingCombosTinkerosRemove: name:"+name+", id:"+id+", price:"+price);	
				}
			} catch (e) {
				console.error(e);
			}
		...
		$("#popup_terms_modal").on('click',function(){
			datalayerMisJugadas(this,'Tu jugada para todos los sorteos','Informarse T&C','Link');
		});

		$("#btn_mobile_comprar_combo_tinka").on('click',function(){
			var jugadas = [$("#results1").text(),null,null,null];
			var importPagar = $("#precioFin").val();
			var jugadasActuales = $("#jugadasActuales").val();
			datalayerMisJugadas(this,'Tu jugada para todos los sorteos','Comprar','Button');
		...
		var thisBtnComprarComboTinka = null;
		
		$("#btn_mobile_comprar_combo_tinka_s").on('click',function(){
			thisBtnComprarComboTinka = this;
			...
			mainValidatePendingsDocsForAproval('interfaceShoppingCartSuscripcionComprarComboTinka');						
		});

		function interfaceShoppingCartSuscripcionComprarComboTinka() {
			var jugadas = [$("#results1").text(),null,null,null];
			var importPagar = $("#precioFin").val();
			var jugadasActuales = $("#jugadasActuales").val();
			document.forms["game_tinka_play_bet_result_suscription_s"].submit();
			datalayerMisJugadas(thisBtnComprarComboTinka,'Tu jugada para todos los sorteos','Comprar','Button');
			datalayerCheckout(jugadas,importPagar/jugadasActuales);
		}

		window.addEventListener("message", function(event) {
		    if (event.data === "interfaceShoppingCartSuscripcionComprarComboTinka") {
		    	interfaceShoppingCartSuscripcionComprarComboTinka(); 
		    }
		});
	</script>
</body>

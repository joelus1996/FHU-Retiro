...
<head>
	...
	<title>Tinka : Realizar apuesta</title>
	<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0">
	<link rel="stylesheet" href="layer-view-style/v2/styles.css?v=<%=Constantes.styles_css%>" type="text/css" />
	<meta name='description' content="Tinka móvil, realiza la apuesta del juego" />
...
<body class="main-tinka" >
	...
	<input type="hidden" id="precioFinal" value="${formatDiscountPayment2}"/>
...
// 				tagging
			...
			taggingCombosTinkerosBet(numeroCombo);
			var jugadas=0;
			...
			$(document).delegate('#btn_mobile_agregar_boleto_tinka', 'click', function() {
				...
				mainValidatePendingsDocsForAproval('interfaceBetSuscripcionAgregarBoletoTinka');
			});
		
			function interfaceBetSuscripcionAgregarBoletoTinka () {
				if(bets==0){
					desplegarModal('No ha realizado ninguna jugada');
					
					var nombreJuego="Error :: Tinka";
					 taggingCombosTinkerosError(nombreJuego,'No ha realizado ninguna jugada');	
					}     
				else {
					var numeroCombo =$("#numeroCombo").val();
					if(numeroCombo!=undefined && numeroCombo!=null && numeroCombo!=''){
						...
						if(jugadas>$limiteJugadas)jugadas=$limiteJugadas;
						if(jugadas>1)price=parseFloat(price)*jugadas;
						taggingSiguienteAddToCart(name,id,price);
					}
			...
			window.addEventListener("message", function(event) {
			    if (event.data === "interfaceBetSuscripcionAgregarBoletoTinka") {
	            	interfaceBetSuscripcionAgregarBoletoTinka(); 
			    }
			});

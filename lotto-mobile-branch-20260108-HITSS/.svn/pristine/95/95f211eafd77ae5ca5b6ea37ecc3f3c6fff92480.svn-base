...
public class CasinoXpgUtils extends Thread {
	...
	public CasinoXpgUtils(String accion, String external_id, SecurityLoginBo beanSecurityLoginBo) {
		this.accion = accion;
		this.external_id = external_id;
		this.username = String.format("%04d", new Long(external_id));
		this.beanSecurityLoginBo = beanSecurityLoginBo;
	}
    ...
    public String getLobbyUrl(String external_id, String game_id, String return_url, String language) {
    	...
    	try {
    		...
    		URL url = new URL(Constantes.CASINO_API_URL+"get/lobby");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constantes.CASINO_API_URL.startsWith("https")) {
	            try {
	                SSLContext sc;
	    	...
	    	params.put("site_id", Constantes.CASINO_SITE_ID);
	    	params.put("access_key", Constantes.CASINO_ACCESS_KEY);
	    	params.put("game_id", game_id);
	    	params.put("external_id", external_id);
	    	params.put("mobile", "1");
	    	params.put("return_url", return_url);
	    	params.put("language", Constantes.CASINO_LANGUAGE);
    ...
    public String getLobbyUrlPreprod(String external_id, String game_id, String return_url, String language) {
    	LoggerApi.Log.info("-------------- START getLobbyUrl preprod");
    	long startTime = System.currentTimeMillis();
    	
    	StringBuilder sbResult = new StringBuilder();
    	try {
    		
    		URL url = new URL(Constantes.PREPROD_CASINO_API_URL+"get/lobby");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constantes.PREPROD_CASINO_API_URL.startsWith("https")) {
	            try {
	                SSLContext sc;
	                sc = SSLContext.getInstance("TLS");
	                sc.init(null, null, new java.security.SecureRandom());
	        ...
	        Map<String,Object> params = new LinkedHashMap<String,Object>();
	    	params.put("site_id", Constantes.PREPROD_CASINO_SITE_ID);
	    	params.put("access_key", Constantes.PREPROD_CASINO_ACCESS_KEY);
	    	params.put("game_id", game_id);
	    	params.put("external_id", external_id);
	    	params.put("mobile", "1");
	    	params.put("return_url", return_url);
    ...
    public String createPlayer(String external_id) {
    	...
    	try {
    		...
    		URL url = new URL(Constantes.CASINO_API_URL+"player/create");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constantes.CASINO_API_URL.startsWith("https")) {
	            try {
	    	...
	    	params.put("currency", Constantes.CASINO_CURRENCY);
	        StringBuilder postData = new StringBuilder();
	    	for (Map.Entry<String,Object> param : params.entrySet()) {
	    		if (postData.length() != 0) postData.append('&');
	    		postData.append(URLEncoder.encode(param.getKey(), "UTF-8"));
    ...
    public String createPlayerPreprod(String external_id) {
    	...
    	try {
    		...
    		URL url = new URL(Constantes.PREPROD_CASINO_API_URL+"player/create");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constantes.PREPROD_CASINO_API_URL.startsWith("https")) {
	            try {
    ...
    public String checkPlayer(String external_id) {
    	...
    	try {
    		...
    		URL url = new URL(Constantes.CASINO_API_URL+"player/check");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constantes.CASINO_API_URL.startsWith("https")) {
	            try {
    ...
    public String checkPlayerPreprod(String external_id) {
    	...
    	try {
    		...
    		URL url = new URL(Constantes.PREPROD_CASINO_API_URL+"player/check");
			HttpURLConnection conn = (HttpURLConnection)url.openConnection();
			// If secure connection
	        if (Constantes.PREPROD_CASINO_API_URL.startsWith("https")) {
	            try {
    ...
    public String processCreatePreprod() {
    	CasinoXpgUtils u = new CasinoXpgUtils(this.accion, this.external_id, this.beanSecurityLoginBo);
    	...
    	if(status.equals("true")) {//existe cliente en XPgame
    		...
    		LoggerApi.Log.info("----CheckPlayer casino XPG preprod player_id: "+player_id);
    	}
    ...
    public String getLobbyUrlQT(String player_id, String game_id, String return_url, String session) {
    	...
    	try {
    		...
    		for (int i=1; i<=2; i++) {	
	    		URL url = new URL(Constantes.CASINO_QT_API_URL+"/v1/games/" + game_id + "/launch-url");
	    		LoggerApi.Log.info("url request lobby="+Constantes.CASINO_QT_API_URL+"/v1/games/" + game_id + "/launch-url");
		    	...
		    	launchRequest.setCurrency(Constantes.CASINO_QT_CURRENCY);
		    	launchRequest.setCountry(Constantes.CASINO_QT_COUNTRY);
		    	launchRequest.setLang(Constantes.CASINO_QT_LANGUAGE);
		    	launchRequest.setMode(Constantes.CASINO_QT_REAL);
		    	launchRequest.setDevice(Constantes.CASINO_QT_DEVICE);
		    	launchRequest.setReturnUrl(return_url);
		    	launchRequest.setWalletSessionId(session);
				
				String json = gson.toJson(launchRequest);		    	
				LoggerApi.Log.info("json request: "+json);
	            ...
	            BufferedReader br = null;
	 			...
	 			if(responseCode < HttpServletResponse.SC_BAD_REQUEST) {
	 				br = new BufferedReader(new InputStreamReader((conn.getInputStream()),Constantes.CHARSET_UTF8));
	 			}else {
	 				br = new BufferedReader(new InputStreamReader((conn.getErrorStream()),Constantes.CHARSET_UTF8));
	 			}
    	...
    	if(this.accion.equals("createPlayer")) {
        	...
        	try {
        		if(!player_id.isEmpty())
        			this.beanSecurityLoginBo.updatePlayerId(Integer.valueOf(this.external_id), player_id);
			} catch (Exception e) {
	            LoggerApi.severe(e);	            
	        } 
    	}    	

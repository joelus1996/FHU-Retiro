...
@Service("beanTinkaResultBo")
public class TinkaResultBoImpl implements TinkaResultBo {
	...
	public List<TinkaList> getResult() throws Exception {
     ...
     try {
    	    LoggerApi.Log.info("");
			objectList= beanViewDAO.findWithCondition07();
			for (TinkaList tinkaList : objectList) {
				tinkaList.setResult(tinkaList.getResult().replace(" ", " - ")+" Boliyapa: "+tinkaList.getExtra1());
				if(tinkaList.getRawid()>5000) tinkaList.setDrwid(tinkaList.getRawid()-5000);
				else tinkaList.setDrwid(tinkaList.getRawid());
				if(tinkaList.getExtra2()!=null)tinkaList.setExtra2(tinkaList.getExtra2().replace(" ", " - "));
			}
			return objectList;

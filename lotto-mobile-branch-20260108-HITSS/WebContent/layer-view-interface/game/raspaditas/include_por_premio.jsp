...
<div class="GridWrapper">								    
    <ul class="GridList" id="listaPorPremio" data-liststyle="thumbnail">								    
	         								                   
	</ul>
</div>
<script type="text/javascript" src="layer-view-script/jquery-3.6.3.min.js"></script>
<script>
$.ajax({ url: 'game_raspaya_productpozo_list.html',
	type: 'get',
	dataType: 'json',
	success: function (response) {
		...
		for(var i = 0; i < listaDetalle.length; i++) {
			...
			itemJuego  = '<li class="GridItem GridItem--match2">';
  			...
  			itemJuego += '<a data-modal="#popup1" href="javascript:botonJuego(\''+ val.gameproductid+'\', \''+ val.name+'\', \''+ val.price+'\', \''+ val.pozo+'\');"  class="js-modal iivv-game" style="background-image: url(\'layer-view-image/game/raspaditas/thumbnails/'+ val.gameproductid +'.png\')"></a>';														
  			itemJuego += '</li>'; 
  			
            $('#listaPorPremio').append(itemJuego);
		}

	}, error: function(e) {
	  	console.log('error obteniendo productos por premio raspaya...')
	}
});
</script>
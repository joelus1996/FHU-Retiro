...
<head>
	...
	<title>Kabala : Gracias por su compra</title>
	<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0">
	<link rel="stylesheet" href="layer-view-style/v2/styles.css?v=<%=Constantes.styles_css%>" type="text/css" />
	<meta name="description" content="Kabala mÃ³vil, gracias por su compra" />
...
<%-- 		<input type="hidden" name="alertPlay"  val="${status}"> --%>
	     ...
	     <input type="hidden" id="chauA" value="${chauA}"/>
	     <input type="hidden" id="chauB" value="${chauB}"/>
	     <input type="hidden" id="chauC" value="${chauC}"/>
	     <input type="hidden" id="chauD" value="${chauD}"/> 
	     ...
	     <input type="hidden" id="jugadasGratis" value="${jugada_gratis_kb}"/>
			...
			<div class="content">
				<section class="main-section">
					<div class="main-game">
						...
						<a href="game_kabala_show_menu.html" id="game_kabala_show_menu">
						...
						<div class="game-desc min-desc">
							...
							<c:if test="${status == 'saldo'}">
								...
								<!-- a href="#" id="kabala-charge" class="btn btn-orange-light">RECARGA TU SALDO</a -->								
								<c:if test="${sessionScope.tipoKabala == 'suscripcion'}">
									<button id="kabala-suscripcion-charge" class="btn btn-orange-light">RECARGA TU SALDO</button>
								</c:if>
								<c:if test="${sessionScope.tipoKabala != 'suscripcion'}">
									<button id="kabala-charge" class="btn btn-orange-light">RECARGA TU SALDO</button>
								</c:if>
							...
							<div class="wrapper-after-play">
								<div class="body-after-play">
									<div class="review-play">
										...
										<!-- a href="#" id="client_play_show_information" class="btn btn-orange-light">MIS JUGADAS</a -->
										<button id="client_play_show_information" class="btn btn-orange-light">MIS JUGADAS</button>
									</div>
							...
							<div class="wrapper-after-play">
								<div class="body-after-play">
									<div class="review-play">
										...
										<!-- a href="#" id="game_kabala_show_shoppingcart" class="btn btn-orange-light">VOLVER</a -->
										<button id="game_kabala_show_shoppingcart" class="btn btn-orange-light">VOLVER</button>
									</div>
								</div>
							</div>
						</c:if>
		...
		$(document).ready(function() {
			...
			if(tipoCompra==="combo"){
				if (status == "ok") {
					...
					taggingCombosKabalaCompraExitosa(numCombo);
					taggingCombosKabalaEEpurchase(numCombo,ticketIdCombo,precioCombo,sorteosCombo,bolillasCombo,tipoJugadaCombo);
					...
					taggingCombosJugadaNoProcesada("Error :: Kabala", alertPlay);
				...
				if (navigation == 0) {
					if (status == "ok") {
						...
						jugadaActuales =  $("#jugadasActuales").val().split(" ")[0];
						var cupon =  "undefined"; 
						/*var precioReal = parseFloat($("#importePagar").val());
						...
						tagginGraciasCompraSlider62();
						tagginPrepararParametrosKabala();
					} else {
						var alertPlay = $("#alertPlay").val();

...
<title>Te Apuesto : Resultados</title>	
...
<meta name='description' content='Te Apuesto móvil, resultado de sorteos anteriores ' />
   ...
   <br>
    ...
    <ul style="margin-right: 5px; margin-left: 5px;" class="listContent listContentCuadro">	
			...
			<c:forEach begin="${rowBEGIN-1}" step="1" end="${rowEnd-1}"
				var="item" varStatus="status" items="${resultGamesTeapuesto}">
			
				<li class="arrow">
				<div style="float: left;" >Cod: ${item[0]}</div>
				<BR>
				<div style="font-size: 11px;"> ${item[1]} - ${item[2]}</div>
				<div style="font-size: 11px;">${item[3]} - ${item[4]}  (${item[5]})</div>
				</li>
			...
			<c:set value="${fn:length(resultGamesTeapuesto)}" var="rLen" />
       ...
       <c:forEach items="${dateResultTeapuesto}" var="item"> 
	    <a href="#" onclick="window.location.href='game_teapuesto_filter_result.html?filter=${fn:substring(item,0,10)}';" 

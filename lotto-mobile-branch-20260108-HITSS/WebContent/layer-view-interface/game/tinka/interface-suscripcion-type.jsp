...
<head>
	...
	<title>Jugar La Tinka Online | Comprar La Tinka en línea</title>
						...
						<!-- esta no debe ser la accion a enviarser 
							...
							<div class="row_trop">
								...
								<div class="right_amarillo">
									<h3 style="pointer-events: none;">Jugada al azar</h3>
									<div class="red_core" style="pointer-events: none;">
								...
								<form action="game_tinka_show_bet_suscripcion.html" id="formdefault">
									...
									<div class="game-actions">
										...
										<c:if test="${!empty clientId && agreement!=''}">
											<c:if test="${indicadorPurchase==negacion}">
												<button id="btn_mobile_comprar_combo_tinka" type="button" class="btn btn-red float-bottom">COMPRAR</button>
											</c:if>
										
											<c:if test="${indicadorPurchase==afirmacion}">
												<form action="game_tinka_play_bet_result_suscription.html" method="post" id ="game_tinka_play_bet_result_suscription">
										...
										<c:if test="${empty clientId or agreement==''}">
											...
											<c:if test="${indicadorPurchase==afirmacion}">
												<form action="security_login_execute_authentication.html">
													...
													<button id="btn_mobile_comprar_combo_tinka_login" type="button" class="btn btn-red float-bottom">COMPRAR</button>
												</form>
											</c:if>
										</c:if>
...
<script type="text/javascript">
		...
		$(document).ready(function () {	
			...
			$(document).delegate('#btn_mobile_agregar_boleto_tinka', 'click', function() {	   
				...
				mainValidatePendingsDocsForAproval('interfaceSuscripcionTypeAgregarBoletoTinka');
			});
			
			$(document).delegate('#btn_mobile_comprar_combo_tinka', 'click', function() {	   
				// Se valida si el usuario tiene documentos pendientes de confirmación, el parámetro que recibe
				// es una function que se realizará en caso no tenga docs pendientes de confirmacion
				mainValidatePendingsDocsForAproval('interfaceSuscripcionTypeComprarComboBoletoTinka');
			});

			window.addEventListener("message", function(event) {
			    if (event.data === "interfaceSuscripcionTypeAgregarBoletoTinka") {
			    	document.forms["game_tinka_add_bet"].submit(); 
			    } else if (event.data === "interfaceSuscripcionTypeComprarComboBoletoTinka") {
			    	document.forms["game_tinka_play_bet_result_suscription"].submit(); 
				}
			});
			...
			$(".right_amarillo").on("click",function(){
				...
				if(numeroCombo!=undefined && numeroCombo!=null && numeroCombo!=''){
					var name="Tinka Combo "+numeroCombo+" jugadas";
					var id="tinkaC"+numeroCombo;
					var price=$("#precioFinalCombo").val();
					taggingSiguienteAddToCart(name,id,price);//paso1
					taggingVirtualPageView2a(numeroCombo);//paso2
					taggingCombosTinkerosEEcheckout(name,id,price);//paso3	
				}
			});
			$(".left_verde").on("click",function(){
				$(".left_verde").removeClass("amarilloF");
				...
				if(clickAzar>0){
					if(numeroCombo!=undefined && numeroCombo!=null && numeroCombo!=''){
						taggingCombosTinkerosRemoveFromCart(name,id,price);	
					}
					clickAzar=0;
				}
			});
			...
			//Taggign
			var numeroCombo=$('#numeroBolilla').val();
			taggingCombosTinkeros(numeroCombo);
	   ...
	   $(document).delegate('#game_tinka_show_menu', 'click', function(e){
		...
		$("#formdefaultButton , #btn_mobile_comprar_combo_tinka").on('click',function(){
			datalayerTinkaJugada(this,'¿Cómo quieres jugar?','Button','Comprar');
		});
		
		$("#btn_mobile_comprar_combo_tinka_login").on('click',function(){
			datalayerTinkaJugada(this,'¿Cómo quieres jugar?','Button','Comprar');

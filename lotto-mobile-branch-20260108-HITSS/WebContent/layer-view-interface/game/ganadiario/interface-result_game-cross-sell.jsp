...
<div class="row" id=list_cross_ganadiario>								        
</div>
...
$.ajax({    		  
	...
	success: function(res) {
	  	...
	  	if (data[0]) {  
	  		var paso ="Paso 3";
			try{
				paso = ($("#SubZona").val() == "Combos Gana Diario") ? 'Paso 4' : 'Paso 3';
			}catch(e){}
					  		
	  		$(res).find('crossSell').children('crossGanadiario').each(function(){
	  			itemJuego  = '<div class="w-50">';
	  			itemJuego += 	'<div class="box-after-play">';
	  			itemJuego += 		'<a href="#" onclick="datalayerGraciasCompra(\''+$(this).find("c_id").text().substr(11,30)+'\',\'seguir jugando\',\'Link\',\''+paso+'\');" id="' + $(this).find("c_id").text() + '">';
	  			itemJuego += 			'<img src="' + $(this).find("c_src").text() + '" alt="" />';															  			
	  			itemJuego += 		'</a>';
	  			itemJuego += 	'</div>';
	  			itemJuego += '</div>'; 
	  			
	            $('#list_cross_ganadiario').append(itemJuego);
	  	    });	  				  		
	  	} else {
	  		console.log('no data')
	  	}

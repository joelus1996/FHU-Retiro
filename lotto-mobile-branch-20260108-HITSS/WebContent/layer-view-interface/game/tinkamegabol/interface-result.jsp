...
<title>Tinka Megabol : Resultados</title>	
...
<meta name='description' content='Tinka Megabol móvil, resultado de sorteos anteriores' />
...
<div align="center" >  
    ...
    <ul style="margin-right: 5px; margin-left: 5px; font-weight: bold;" class="listContent listContentCuadro">	
      <c:forEach	var="item"  items="${getListResult}">	     
		<li class="arrow">							      
		   <div align="center" style="font-size: 11px">
		   Sorteo ${item.rawid} - ${item.date} <BR>
		   ${item.result}  <BR>
		   Megabolilla: ${item.extra1}
		   <c:if test="${!empty item.extra2}">
		    <BR> Adicional: ${item.extra2}
		   </c:if>
		   </div> 			
		</li>
		</c:forEach>

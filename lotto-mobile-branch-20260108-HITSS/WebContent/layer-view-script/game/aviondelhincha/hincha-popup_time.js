function activarPopupHincha(flag,tipo){
	...
	function checkcount_hincha(tipo) {
		var count_hincha_mobile = GetCookie('count_hincha_mobile_'+tipo);
		if (count_hincha_mobile == null) {
			var expDays = 1;
			localStorage.removeItem("exp_hincha_mobile_"+tipo);
			var exp = new Date();
			exp.setTime(exp.getTime() + (expDays*24*60*60*1000));
			count_hincha_mobile=1;
			SetCookie('count_hincha_mobile_'+tipo, count_hincha_mobile, exp);
			localStorage.setItem("exp_hincha_mobile_"+tipo, exp);
			$.ajax({
		        type: "POST",
			...
			count_hincha_mobile++;
			flag_count = count_hincha_mobile;
			var auxExp = localStorage.getItem("exp_hincha_mobile_"+tipo);
			try{
				SetCookie('count_hincha_mobile_'+tipo, count_hincha_mobile, auxExp);
			}catch (e) {
				if (flag=='1')
					closeModal(1);
				else
...
$(document).delegate('#btn_mobile_informate_hincha', 'click', function () {			
	window.location.href = $('#urlQw').val() + '/promociones/el-avion-del-hincha/?origin=i';
});

$(document).delegate('#btn_mobile_participa_hincha', 'click', function () {			
	 $.ajax({
	        type: "POST",
...
$(document).delegate('#btn_mobile_participa_hincha_ta', 'click', function () {			

...
if(temp != '-') ajackpotids = temp.slice(0, -1).split(',');
...
function botonJuego(gameId, gameDemo, mainp){
	...
	$(document).ready(function() {
		...
		if( dataGameId != "undefined"){	
			...
			$("#btnJuega").attr("data-href","game_casino_show_home.html?game="+ dataGameId +"&mode=live&mainp=" + mainp);
...
function ocultaSubtitulo(){
	...
	$(document).ready(function(){
       ...
       $(".casino-overlay").on("click",function(){
			...
			if( dataGameId != undefined){
				$("#btnDemo").attr("data-href",dataGameDemo);
				$("#btnJuega").attr("data-href","game_casino_show_home.html?game="+ dataGameId +"&mode=live");
				$('#popup1').addClass('opened');
			}				
		});
	...
	$("#btnDemo").click(function () {
        ...
        var url = '';
        if(value.indexOf("mode=demo") == -1) url = "game_casino_show_home.html?demo="+ value;
        else url = value;
   ...
   function actualizaProveedor(tipo, prov){
	...
	function armaItemJuego(val){
		...
		if( val.demolink === undefined ){
			...
			if( val.provname != 'Hacksaw' ){
				if(val.type == 'live')
					vbotonJuego = ' href="game_casino_show_home.html?game='+ val.gameproductid +'&amp;mode=live&amp;category='+ val.type + '&amp;mainp='+ val.mainprovider + '" ';
				else
					vbotonJuego = ' href="game_casino_show_home.html?game='+ val.gameproductid +'&amp;mode=live' + '&amp;mainp='+ val.mainprovider + '" ';
	...
	$.ajax({ url: 'game_casino_product_list.html',
		type: 'get',
		dataType: 'json',
		success: function (response) {
			  	jsonProds = response;
	...
	$.ajax({ url: 'game_casino_product_list_order.html',
		type: 'get',
		dataType: 'json',
		success: function (response) {
			  	jsonProdsFix = response;
			}, error: function(e) {
			  	console.log('error obteniendo productos casino orderfix...')
			}
	});
	 ...
	 function actualizaMasJugados(){
		var item = '';
		$.ajax({ url: 'game_casino_most_played_list.html',
			type: 'get',
			dataType: 'json',
			success: function (response) {
				listaDetalle = response;

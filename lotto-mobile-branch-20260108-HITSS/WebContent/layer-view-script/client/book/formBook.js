		...
		if(img!=undefined && img!=null){
		  if(img.type=="image/png" || img.type=="image/jpeg"){
			  if((img.size/1024/1024)<=10){  
			      if ($(this).val() !== '') { 
				    ...
				    imgCanvas.onload = function(){
				      ...
				      imgCanvas.onerror = function() {
						...
						filenames.addClass('empty');
						ImgBase64Array = ["","",""];
						alertMessage("Debes adjuntar imágenes v&aacute;lidas");
				  	  }
				  ...
				  filenames.addClass('empty');
				  ImgBase64Array = ["","",""];
				  alertMessage("Las imágenes deben ser menor a 10MB.");
			  }
		  }else{
			  file.val('');
			  filenames.addClass('empty');
...
$('#enviar').click(function(){
	...
	if(response.length !=0){
	  ...
	  $.ajax({
		...
		alertMessage('Por seguridad, debes seleccionar el código captcha');
	}
});

package pe.com.intralot.loto.layer.view.game.copabicolor;
...
public class CopaBicolorController {
	...
	public ModelAndView registrarPromo(HttpServletRequest request, ModelMap objectModelMap) throws Exception {
		...
		if(!idClient.isEmpty()) {
	        ...
	        try {
	            rs = new Dbms();
	    	    String sql = "update lotocard.client set status_prom_bicolor = ? , cl_date_prom_bicolor = sysdate , cl_channel_prom_bicolor = 'MOBILE' where client_id = ? and status_prom_bicolor is null ";
	    	    rs.setSql(sql);
	            rs.setString(1,"1");
	            rs.setString(2,idClient);
	            rs.executeUpdate();
	...
	public ModelAndView showcopabicolor(HttpServletRequest request, ModelMap objectModelMap) throws Exception {
		...
		try {
			...
			if(session.getAttribute("clientId") != null) {
	    		...
	    		int idClientPromo = Integer.parseInt(idClient);
	    		flagPromo = verifyPromo(idClientPromo);
	...
	public ModelAndView showAmLegend(HttpServletRequest request, ModelMap objectModelMap) throws Exception {
		...
		try {
			...
			objectModelMap.put("clientId", idClient);
			objectModelMap.put("flagPromo", flagPromo);
			return new ModelAndView("game/copabicolor/interface-legend");	
	...
	@RequestMapping("/premiazoganagol_resultados")	
	public ModelAndView resulstHouseCup(HttpServletRequest request, ModelMap modelList) throws Exception {
		...
		try {			
			...
			List<ClientProcedureGetBicolorPromoList> list = clientBalanceBo.getBicolorPromoAccount(clientId);
								
			String pattern = "dd ' de ' MMMMM";			
			SimpleDateFormat simpleDateFormat =
			        new SimpleDateFormat(pattern, new Locale("es","PE"));
			
			boolean verifyPromo = verifyPromo(clientId);
			if(!list.isEmpty() && clientId!=0 && verifyPromo == true) {
	...
	@RequestMapping("/game_premiazoganagol_resultados_popup")	
	public void resulstHouseCupPopup(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try {			
			...
			if(!list.isEmpty() && clientId!=0) {
				...
				puntajeNivel = (int) Math.round(puntaje);
				List<ListTicketBicolorClient> listTicketsWin = clientBalanceBo.getListBicolorTickets(clientId);
	...
	public void validarClientePromocion(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		String idClient = "";
		idClient = session.getAttribute("clientId").toString().trim();
		String flagPromoBicolor = Constantes.flagPromoBicolor;
		...
		int idClientPromo = Integer.parseInt(idClient);
			...
			boolean verifyPromo = verifyPromo(idClientPromo);
	...
	public boolean verifyPromo(int idClient)  throws Exception {
		...
		boolean flagPromo = false;
		String idClientPromo = Integer.toString(idClient);//revisar cuando llega cliente vacio
	...
	public void validarClientePromocionHincha(HttpServletRequest request,HttpServletResponse response) throws Exception {
	    ...
	    PrintWriter out = response.getWriter();
		int idClient = 0;
		String flagPromoHincha = Constantes.flagPromoHincha;
		boolean verifyPromo = false;

		if(session.getAttribute("clientId") != null) {
    		idClient = Integer.parseInt(session.getAttribute("clientId").toString().trim());
    	}
	...
	public void registrarPromoHincha(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(session.getAttribute("clientId") != null) {
    		idClient = Integer.parseInt(session.getAttribute("clientId").toString().trim());
    		idClientPromo = session.getAttribute("clientId").toString().trim();
    	}
			
		if(!idClientPromo.isEmpty()) {			
			clientSaleBo.registerPopupLottery(idClient, "Mobile", "H", Game.TEAPUESTO);												
		}
	}
}
